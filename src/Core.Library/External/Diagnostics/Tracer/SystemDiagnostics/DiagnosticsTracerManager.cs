namespace System.Net.Mqtt.Diagnostics
{
	...
	public class DiagnosticsTracerManager : ITracerManager, IDisposable
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="DiagnosticsTracerManager"/> class.
        /// </summary>
        public DiagnosticsTracerManager()
        {
        ...
        {
            private DiagnosticsTracerManager manager;
            private List<DiagnosticsTracer> tracers;
            private string name;

            public AggregateTracer(DiagnosticsTracerManager manager, string name, IEnumerable<DiagnosticsTracer> tracers)
            {
        ...
        partial class DiagnosticsTracer
        {
			...
			private TraceEventTypeAdapter eventTypeAdapter;

			public DiagnosticsTracer(TraceSource source)
            {
                this.source = source;
				this.eventTypeAdapter = new TraceEventTypeAdapter ();
            }
            ...
            public void Trace(string sourceName, TraceEventType type, object message)
            {
                ...
                using (new SourceNameReplacer(source, sourceName))
                {
                    ...
                    if (data != null)
                        source.TraceData(eventTypeAdapter.Get (type), 0, data);
                    else if (guid != null && type == TraceEventType.Transfer)
                        source.TraceTransfer(0, "", guid.Value);
                    else
                        source.TraceEvent(eventTypeAdapter.Get (type), 0, message.ToString());
                }
            }
            ...
            public void Trace(string sourceName, TraceEventType type, string format, params object[] args)
            {
                ...
                using (new SourceNameReplacer(source, sourceName))
                {
                    source.TraceEvent(eventTypeAdapter.Get (type), 0, format, args);
                }
            }
            ...
            public void Trace(string sourceName, TraceEventType type, Exception exception, object message)
            {
                ...
                using (new SourceNameReplacer(source, sourceName))
                {
                    source.TraceEvent(eventTypeAdapter.Get (type), 0, message.ToString() + Environment.NewLine + exception);
                }
            }
            ...
            public void Trace(string sourceName, TraceEventType type, Exception exception, string format, params object[] args)
            {
                ...
                using (new SourceNameReplacer(source, sourceName))
                {
                    source.TraceEvent(eventTypeAdapter.Get (type), 0, string.Format(format, args) + Environment.NewLine + exception);
                }
            }
