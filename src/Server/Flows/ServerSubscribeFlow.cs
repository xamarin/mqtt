			...
			foreach (var subscription in subscribe.Subscriptions) {
				try {
					if (!this.topicEvaluator.IsValidTopicFilter (subscription.TopicFilter)) {
						tracer.Error(LogMessage.Create(Resources.Tracer_ServerSubscribeFlow_InvalidTopicSubscription, subscription.TopicFilter, clientId));
					...
					tracer.Error(repoEx, LogMessage.Create(Resources.Tracer_ServerSubscribeFlow_ErrorOnSubscription, clientId, subscription.TopicFilter));
